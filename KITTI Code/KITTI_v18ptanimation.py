...
VIDEO_OUTPUT_PATH  = os.path.join("D:\\Stingray Robotics", "sift_pointcloud_topdown.mp4")
VIDEO_FPS          = 60
...
if len(pointcloud_world_est) > 0 and len(pc_est_all) > 0:
    ...
    canvas = np.zeros((canvas_h, canvas_w, 3), dtype=np.uint8)  # persistent - points accumulate on it
    ...
    colormap    = plt.cm.viridis  # dark (early) -> bright yellow (late): encodes path progression
