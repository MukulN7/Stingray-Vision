...
# SIFT parameters
MIN_FEATURES = 350          # Min features to keep tracking before re-detecting SIFT
...
# Detect SIFT features of first frame
sift = cv2.SIFT_create()
keypoints, descriptors = sift.detectAndCompute(left_frame_old, None)

# Extracting keypoint coordinates into pts_old
...
for frame_idx in range(START_FRAME + 1, END_FRAME):
    ...
    pts_new, status, error = cv2.calcOpticalFlowPyrLK(left_frame_old, left_frame_new, pts_old, None)


    # Track Filtering


    # Filtering based on LK status
    goodpts_old = pts_old[status == 1]
    goodpts_new = pts_new[status == 1]

    # Filtering based on RANSAC
    ...
    # Updating previous frame to the current frame
    left_frame_old = left_frame_new

    # Updating pts_old to the successfully tracked points in the new frame
    pts_old = goodpts_new.reshape(-1, 1, 2)

    # Re-detect SIFT if too few tracked points remain
