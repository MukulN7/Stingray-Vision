...
for frame_idx in tqdm(range(START_FRAME + 1, END_FRAME), desc="Processing frames"):
    ...
    if len(sp_depths) >= 6:  # 6 correspondences = 12 equations for the 6-DOF solve
    ...
    if len(sift_depths) >= 6:  # 6 correspondences = 12 equations for the 6-DOF solve
...
# Gradient colormaps: calculated trajectory goes blue -> red over time,
# ground truth trajectory goes dark pink -> yellow over time
calc_cmap = LinearSegmentedColormap.from_list("calc_blue_red", ["blue", "red"])
gt_cmap   = LinearSegmentedColormap.from_list("gt_pink_yellow", ["darkmagenta", "yellow"])

def gradient_line(ax, x, z, cmap, label):
    ...
    # Proxy artist so the gradient line still shows up in the legend
    ax.plot([], [], color=cmap(0.5), label=label)

def plot_trajectory(title, x_est, z_est, color):
    fig, ax = plt.subplots(figsize=(7, 7))
    gradient_line(ax, x_est,     z_est,     calc_cmap, label=title)
    gradient_line(ax, x_gt_hist, z_gt_hist, gt_cmap,   label="Ground Truth")
