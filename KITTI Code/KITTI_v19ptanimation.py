...
# SIFT 3D Point Cloud Video (top-down, estimated-pose points only, no GT, cumulative)
VIDEO_OUTPUT_PATH  = os.path.join(os.path.dirname(POSES), "sift_pointcloud_topdown.mp4")
VIDEO_FPS          = 20
VIDEO_CANVAS_SIZE  = (900, 900)  # (width, height) in pixels
VIDEO_POINT_RADIUS = 2
VIDEO_MARGIN_PX    = 40          # padding around the point cloud extent
...
# SIFT 3D Point Cloud Video (top-down, estimated-pose points only, cumulative)
# Renders each frame's newly back-projected SIFT points onto a persistent top-down
# (X-Z) canvas. Points are never cleared, so the growing cloud traces the path the
# camera followed. Color encodes time-of-addition (colormap start -> end) so travel
# direction stays readable even once the cloud overlaps itself. No GT points here.


if len(pointcloud_world_est) > 0 and len(pc_est_all) > 0:

    # World-frame X/Z bounds across the whole estimated cloud (camera frame: X right, Z forward)
    x_all = pc_est_all[:, 0]
    z_all = pc_est_all[:, 2]
    ...
    usable_w = canvas_w - 2 * VIDEO_MARGIN_PX
    usable_h = canvas_h - 2 * VIDEO_MARGIN_PX

    x_range = max(x_max - x_min, 1e-6)
    z_range = max(z_max - z_min, 1e-6)
    scale   = min(usable_w / x_range, usable_h / z_range)  # equal aspect, like ax.axis("equal")

    x_offset = VIDEO_MARGIN_PX + (usable_w - x_range * scale) / 2.0
    z_offset = VIDEO_MARGIN_PX + (usable_h - z_range * scale) / 2.0

    def world_to_pixel(x, z):
        # Z increases "up" in the video (forward = up), X increases right
        px = int(x_offset + (x - x_min) * scale)
        py = int(canvas_h - (z_offset + (z - z_min) * scale))
        return px, py

    canvas = np.full((canvas_h, canvas_w, 3), 255, dtype=np.uint8)  # white background, persistent - points accumulate on it

    fourcc       = cv2.VideoWriter_fourcc(*"mp4v")
    video_writer = cv2.VideoWriter(VIDEO_OUTPUT_PATH, fourcc, VIDEO_FPS, (canvas_w, canvas_h))

    total_steps = len(pointcloud_world_est)
    colormap    = plt.cm.coolwarm  # blue (early) -> red (late): encodes path progression, stays visible on white

    for i, frame_pts in enumerate(tqdm(pointcloud_world_est, desc="Rendering SIFT point cloud video")):
        t = i / max(total_steps - 1, 1)
        r, g, b, _ = colormap(t)
        color_bgr  = (int(b * 255), int(g * 255), int(r * 255))

        for pt in frame_pts:
            px, py = world_to_pixel(pt[0], pt[2])
            if 0 <= px < canvas_w and 0 <= py < canvas_h:
                cv2.circle(canvas, (px, py), VIDEO_POINT_RADIUS, color_bgr, -1)

        video_writer.write(canvas)

    video_writer.release()
    print(f"Saved SIFT point cloud top-down video to: {VIDEO_OUTPUT_PATH}")
else:
    print("No SIFT point cloud data available - skipping video export.")
