...
# 3D point cloud accumulator (SIFT keypoints back-projected + placed in world frame, estimated pose only)
pointcloud_world_est = []  # placed using GT rotation + SIFT+LP estimated position

# Live 3D Point Cloud Visualization (Open3D) - window opens now and updates as frames are processed
pcd_est = o3d.geometry.PointCloud()
pcd_est.paint_uniform_color([0.2, 0.8, 0.2])  # green: SIFT+LP estimated-pose reconstruction
vis = o3d.visualization.Visualizer()
vis.create_window(window_name="SIFT 3D Point Cloud: Estimated (green) - Live")
vis.add_geometry(pcd_est)
pointcloud_view_reset = False  # reset the camera once, the first time real points land
...
for frame_idx in tqdm(range(START_FRAME + 1, END_FRAME), desc="Processing frames"):
    ...
    if len(sift_depths_new) > 0 and (frame_idx - START_FRAME) % POINTCLOUD_FRAME_STRIDE == 0:
        ...
        if len(pc_pts_2d) > 0:
            ...
            # Live update: push the growing estimated point cloud to the Open3D window
            pcd_est.points = o3d.utility.Vector3dVector(np.vstack(pointcloud_world_est))
            vis.update_geometry(pcd_est)
            vis.poll_events()
            vis.update_renderer()
            if not pointcloud_view_reset:
                vis.reset_view_point(True)
                pointcloud_view_reset = True
...
# 3D Point Cloud Reconstruction (SIFT keypoints, Open3D) - already plotted live during the main loop above
# Keep the window open after processing finishes so it stays movable/interactive
