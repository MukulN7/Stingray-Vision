...
# Per-frame timestamps come directly from the dataset's image list, not from
# the AVI container's declared FPS (which may not reflect true capture timing
# and hides frame-drop gaps). frame_times[i] is the real acquisition time of
# the i-th left frame in the video, used both for dt and for GPS lookup.
with open(IMAGES_FILE, "r") as f:
    frame_times = np.array([float(line.strip().split("_")[2])
                             for line in f if "_left" in line])
t0 = frame_times[0]
n_frame_times = len(frame_times)
...
n_video_frames = int(cap_left.get(cv2.CAP_PROP_FRAME_COUNT))
if n_video_frames > 0 and n_video_frames != n_frame_times:
    print(f"WARNING: video has {n_video_frames} frames but IMAGES_FILE lists "
          f"{n_frame_times} left-frame timestamps. Frame-to-timestamp sync may "
          f"be off if these don't match 1:1.")

# frame_idx tracks which entry in frame_times corresponds to the frame we're
# about to read; dt is computed per-frame from real consecutive timestamps
# (this also naturally reflects any dropped-frame gaps in the recording).
frame_idx = 0

ret_l, left_raw = cap_left.read()
left_frame_old  = cv2.cvtColor(left_raw, cv2.COLOR_BGR2GRAY)
frame_idx = 1   # the *next* frame read in the main loop will be frame_times[1]
...
with open(OUTPUT_FILE, "w") as log:
    ...
    while True:
        ...
        if frame_idx >= n_frame_times:
            print("Ran out of timestamps for remaining video frames, stopping.")
            break

        # Real inter-frame interval from the dataset's own timestamps, not an
        # assumed constant. This also correctly reflects any dropped-frame
        # gaps instead of silently treating them as a normal-length frame.
        dt = frame_times[frame_idx] - frame_times[frame_idx - 1]
        curr_frame_t = frame_times[frame_idx]
        curr_time = curr_frame_t - t0
        ...
        gt_pos_curr = gt_position(curr_frame_t)
