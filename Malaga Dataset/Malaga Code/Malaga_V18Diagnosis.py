...
CALIB        = r"D:\Stingray Robotics\Malaga\Malaga Urban Dataset\camera_params_raw_1024x768.txt"  # <-- point this at your local copy
...
# ---------------------------------------------------------------------------
# Matches the real camera_params_raw_1024x768.txt layout:
#   [CAMERA_PARAMS_LEFT]  / [CAMERA_PARAMS_RIGHT]
#       cx, cy, fx, fy   each on their own line
#       dist = [K1 K2 T1 T2 K3]   (single bracketed 5-vector, in that order)
#   [CAMERA_PARAMS_LEFT2RIGHT_POSE]
#       translation_only     = [tx ty tz]
#       rotation_matrix_only = [r11 r12 r13 ; r21 r22 r23 ; r31 r32 r33]

with open(CALIB, "r") as f:
    calib_text = f.read()

left_section  = calib_text.split("[CAMERA_PARAMS_LEFT]")[1].split("[CAMERA_PARAMS_RIGHT]")[0]
right_section = calib_text.split("[CAMERA_PARAMS_RIGHT]")[1]
# cut off right_section at the next section header (if any) so later regexes
# (rotation_matrix_only / translation_only, which live outside both blocks)
# don't accidentally get grabbed out of the right block by mistake.
right_section_only = re.split(r"\n\[", right_section)[0]


def parse_field(pattern, text, name):
    m = re.search(pattern, text)
    if m is None:
        raise ValueError(f"Could not find field '{name}' in calibration text "
                          f"using pattern: {pattern}")
    return float(m.group(1))


def parse_intrinsics(section):
    fx = parse_field(r"fx\s*=\s*([\d.eE+-]+)", section, "fx")
    fy = parse_field(r"fy\s*=\s*([\d.eE+-]+)", section, "fy")
    cx = parse_field(r"cx\s*=\s*([\d.eE+-]+)", section, "cx")
    cy = parse_field(r"cy\s*=\s*([\d.eE+-]+)", section, "cy")
    return fx, fy, cx, cy


def parse_distortion(section):
    # dist = [K1 K2 T1 T2 K3]  -- one bracketed 5-vector, in that order
    m = re.search(r"dist\s*=\s*\[([^\]]+)\]", section)
    if m is None:
        raise ValueError("Could not find 'dist = [...]' field in calibration text.")
    vals = [float(v) for v in re.split(r"\s+", m.group(1).strip()) if v]
    if len(vals) != 5:
        raise ValueError(f"Expected 5 distortion values (K1 K2 T1 T2 K3), got {len(vals)}: {vals}")
    return np.array(vals, dtype=np.float64)  # [K1, K2, T1, T2, K3]


fx_l, fy_l, cx_l, cy_l = parse_intrinsics(left_section)
fx_r, fy_r, cx_r, cy_r = parse_intrinsics(right_section_only)

dist_left  = parse_distortion(left_section)
dist_right = parse_distortion(right_section_only)
...
# Extrinsics between the two cameras (right camera relative to left camera)
translation = re.search(r"translation_only\s*=\s*\[([^\]]+)\]", calib_text).group(1).split()
T = np.array([float(v) for v in translation], dtype=np.float64).reshape(3, 1)
baseline_raw = abs(T[0, 0])

rot_match = re.search(r"rotation_matrix_only\s*=\s*\[([^\]]+)\]", calib_text, re.DOTALL)
if rot_match is None:
    raise ValueError("Could not find 'rotation_matrix_only' in calibration text.")
rot_values = re.split(r"[\s;,]+", rot_match.group(1).strip())
rot_values = [v for v in rot_values if v != ""]
R = np.array([float(v) for v in rot_values], dtype=np.float64).reshape(3, 3)

# ---- SUGGESTION #2 CHECK: calibration / baseline sanity ----
print("=" * 60)
print("[CHECK] Calibration values loaded from CALIB file:")
print(f"        fx_left={fx_l:.4f}  fy_left={fy_l:.4f}  cx_left={cx_l:.4f}  cy_left={cy_l:.4f}")
print(f"        fx_right={fx_r:.4f} fy_right={fy_r:.4f} cx_right={cx_r:.4f} cy_right={cy_r:.4f}")
print(f"        dist_left  = {dist_left}")
print(f"        dist_right = {dist_right}")
print(f"        baseline (raw, from translation_only) = {baseline_raw:.6f} m")
if baseline_raw < 0.05 or baseline_raw > 0.5:
...
def compute_depth(goodpts_new, disparity, fx, baseline):
    depths = []
...
def compute_velocity(goodpts_old, goodpts_new, depths, dt, fx, cx, cy):
...
frame_w = int(cap_left.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_h = int(cap_left.get(cv2.CAP_PROP_FRAME_HEIGHT))
image_size = (frame_w, frame_h)
print(f"[CHECK] Video frame size detected: {image_size}")
...
# ---------------------------------------------------------------------------
# Stereo Rectification (done once, at startup)
# ---------------------------------------------------------------------------
...
# After rectification both cameras share the same fx/fy/cy in their P matrices
# (P1 and P2 differ only in the Tx term baked into P2[0,3]). Use the RECTIFIED
# intrinsics from here on, not the raw K_left ones, since disparity / optical
# flow are now computed in the rectified image planes.
fx = P1[0, 0]
fy = P1[1, 1]
cx = P1[0, 2]
cy = P1[1, 2]
...
print("[CHECK] Post-rectification intrinsics/baseline used for depth & velocity:")
print(f"        fx={fx:.4f} fy={fy:.4f} cx={cx:.4f} cy={cy:.4f} baseline={baseline:.6f} m")


def rectify_left(gray_img):
    return cv2.remap(gray_img, map1_left, map2_left, cv2.INTER_LINEAR)


def rectify_right(gray_img):
    return cv2.remap(gray_img, map1_right, map2_right, cv2.INTER_LINEAR)
...
left_frame_old  = rectify_left(left_frame_old)
...
while True:
    ...
    # Rectify BOTH views before anything else touches them. Everything
    # downstream (stereo matching, SIFT, optical flow, depth/velocity math)
    # now operates in the rectified image planes, consistent with the
    # rectified fx/cx/cy/baseline computed above.
    left_frame_new = rectify_left(left_frame_new)
    right_img      = rectify_right(right_img)
    ...
    sift_depths = compute_depth(sift_new, disparity, fx, baseline)
    ...
    if len(sift_depths) > 0:
        vx_sift, vy_sift, vz_sift = compute_velocity(sift_old, sift_new, sift_depths, dt, fx, cx, cy)
