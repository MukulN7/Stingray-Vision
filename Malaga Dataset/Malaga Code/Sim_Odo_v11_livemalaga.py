...
def draw_overlay(img_bgr, true_vals, sp_vals, sift_vals):
    ...
    put("SuperPoint (MA)", (0, 255, 0), bold=True)
    put(f"  Vx={sp_vals[0]:+.3f} Vy={sp_vals[1]:+.3f} Vz={sp_vals[2]:+.3f}")
    put("SuperPoint (LP)", (0, 255, 0), bold=True)
    put(f"  Vx={sp_vals[3]:+.3f} Vy={sp_vals[4]:+.3f} Vz={sp_vals[5]:+.3f}")
...
while True:
    ...
    display_frame = draw_overlay(
        display_frame,
        (tvx, tvy, tvz, tspeed),
        (vx_sp_ma, vy_sp_ma, vz_sp_ma, vx_sp_lp, vy_sp_lp, vz_sp_lp),
