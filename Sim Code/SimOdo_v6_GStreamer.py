...
from cv2_reciever import CameraStreamer


# Configuration


CALIB       = "robotsimcalib.txt"
MIN_FEATURES = 400
OUTPUT_FILE  = "velocities.txt"
...
# Velocity overlay helper


def draw_velocity_overlay(frame, vx, vy, vz):
    overlay = frame.copy()
    lines = [
        f"Vx: {vx:.3f} m/s",
        f"Vy: {vy:.3f} m/s",
    ...
    for i, text in enumerate(lines):
        cv2.putText(overlay, text, (10, 30 + i * 28),
...
camera_left  = CameraStreamer(pipeline1, "Left")
camera_right = CameraStreamer(pipeline2, "Right")

t1 = threading.Thread(target=camera_left.run)
t2 = threading.Thread(target=camera_right.run)
t1.start()
t2.start()
...
while True:
    left_raw  = camera_left.get_frame()
    right_raw = camera_right.get_frame()
...
# Current velocity state for overlay (start at zero)
vx_sp_ma = vy_sp_ma = vz_sp_ma = 0.0
vx_sift_ma = vy_sift_ma = vz_sift_ma = 0.0
...
try:
    with open(OUTPUT_FILE, "w") as log:
        log.write("timestamp,vx_sp_ma,vy_sp_ma,vz_sp_ma,vx_sp_lp,vy_sp_lp,vz_sp_lp,vx_sift_ma,vy_sift_ma,vz_sift_ma,vx_sift_lp,vy_sift_lp,vz_sift_lp\n")
        ...
        while True:
            ...
            log.write(f"{curr_time:.6f},{vx_sp_ma:.6f},{vy_sp_ma:.6f},{vz_sp_ma:.6f},"
                      f"{vx_sp_lp:.6f},{vy_sp_lp:.6f},{vz_sp_lp:.6f},"
                      f"{vx_sift_ma:.6f},{vy_sift_ma:.6f},{vz_sift_ma:.6f},"
                      f"{vx_sift_lp:.6f},{vy_sift_lp:.6f},{vz_sift_lp:.6f}\n")
            log.flush()

            print(f"dt={dt*1000:.1f}ms | SP  MA=({vx_sp_ma:.3f},{vy_sp_ma:.3f},{vz_sp_ma:.3f})  LP=({vx_sp_lp:.3f},{vy_sp_lp:.3f},{vz_sp_lp:.3f})"
                  f" | SIFT  MA=({vx_sift_ma:.3f},{vy_sift_ma:.3f},{vz_sift_ma:.3f})  LP=({vx_sift_lp:.3f},{vy_sift_lp:.3f},{vz_sift_lp:.3f})")


            # Show Camera Windows with Velocity Overlay (SP on left, SIFT on right)


            cv2.imshow("Left Camera  [SuperPoint]",  draw_velocity_overlay(left_raw,  vx_sp_ma,   vy_sp_ma,   vz_sp_ma))
            cv2.imshow("Right Camera [SIFT]",        draw_velocity_overlay(right_raw, vx_sift_ma, vy_sift_ma, vz_sift_ma))
