...
Output convention: vx/vy/vz in the output CSVs are intended to be
North/East/Down (nav frame), matching gt_N/gt_E/gt_D in
stereo_pose_est.data directly. This holds only once the camera-to-body
axis mapping below is correct for your rig -- see find_axis_mapping.py.
"""
...
VELOCITY_CSV_OUT = r"D:\Stingray Robotics\SR Output\SR Output 2\velocity_output0010_scottreef_rotated.csv"
POSITION_CSV_OUT = r"D:\Stingray Robotics\SR Output\SR Output 2\position_output0010_scottreef_rotated.csv"
...
for i in range(n):
    ...
    for v in VARIANTS:
        ...
        # NOTE: v_cam is in camera axes (X-right, Y-down, Z-forward/optical).
        # This rig is nadir-facing, so the camera-to-body axis mapping is
        # NOT the simple forward-facing assumption. Run find_axis_mapping.py
        # to determine the correct permutation/signs for THIS mount, then
        # apply it here (currently passing v_cam straight through unmapped).
        v_body = v_cam
        v_nav = R @ v_body
