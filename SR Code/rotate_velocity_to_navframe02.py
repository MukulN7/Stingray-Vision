...
Output convention: vx/vy/vz in the output CSVs are North/East/Down (nav
frame), matching gt_N/gt_E/gt_D in stereo_pose_est.data directly -- no
further rotation is needed to compare against ground truth.
"""
...
VELOCITY_CSV_OUT = r"D:\Stingray Robotics\SR Output\SR Output 2\velocity_output0021_scottreef_rotated.csv"
POSITION_CSV_OUT = r"D:\Stingray Robotics\SR Output\SR Output 2\position_output0021_scottreef_rotated.csv"
...
for i in range(n):
    ...
    for v in VARIANTS:
        ...
        # v_cam is in camera axes: X-right, Y-down, Z-forward.
        # rotation_matrix() expects body axes: X-forward, Y-right, Z-down.
        # Remap before rotating, or the DCM rotates the wrong components.
        v_body = np.array([-1 * v_cam[1], +1 * v_cam[0], +1 * v_cam[2]])
        v_nav = R @ v_body
