...
VELOCITY_CSV_PATH = r"D:\Stingray Robotics\velocity_output_scottreef5.csv"
POSITION_CSV_PATH = r"D:\Stingray Robotics\position_output_scottreef5.csv"
...
# Shared: Depth + Rigid-Fit Velocity helpers
...
def rigid_transform(pts_old, pts_new):
    # Kabsch/Umeyama: SVD solve for the rotation + translation that best maps
    # pts_old onto pts_new (real 3D points, not image-plane motion)
    centroid_old = pts_old.mean(axis=0)
    centroid_new = pts_new.mean(axis=0)
    H = (pts_old - centroid_old).T @ (pts_new - centroid_new)
    U, _, Vt = np.linalg.svd(H)
    d = np.sign(np.linalg.det(Vt.T @ U.T))
    Rm = Vt.T @ np.diag([1, 1, d]) @ U.T
    t  = centroid_new - Rm @ centroid_old
    return Rm, t

def compute_velocity(goodpts_old, goodpts_new, depths_old, depths_new, dt):
    # Back-project matched 2D points + depth into real 3D points (camera frame)
    pts3d_old = np.column_stack([(goodpts_old[:, 0] - cx) * depths_old / fx,
                                  (goodpts_old[:, 1] - cy) * depths_old / fx,
                                  depths_old])
    pts3d_new = np.column_stack([(goodpts_new[:, 0] - cx) * depths_new / fx,
                                  (goodpts_new[:, 1] - cy) * depths_new / fx,
                                  depths_new])

    Rm, t = rigid_transform(pts3d_old, pts3d_new)

    # Added: drop points with a large residual after the first solve, then re-solve
    # (kills the single-bad-correspondence spikes without touching the rigid-fit model)
    residuals = pts3d_new - (pts3d_old @ Rm.T + t)
    ...
    if len(residual_norms) > 6:   # keep enough points left to solve for 6 DOF
        ...
        if keep.sum() >= 6 and keep.sum() < len(keep):
            Rm, t = rigid_transform(pts3d_old[keep], pts3d_new[keep])
