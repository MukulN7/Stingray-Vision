...
VEL_CSV   = r"D:\Stingray Robotics\SR Output\SR Output 2\velocity_output0024_scottreef_rotated.csv"
POS_CSV   = r"D:\Stingray Robotics\SR Output\SR Output 2\position_output0024_scottreef_rotated.csv"
...
# --- Velocity plots (one figure per method, GT and estimate on separate subplots) ---
for m in methods:
    fig, axs = plt.subplots(3, 2, figsize=(14, 7), sharex=True)
    fig.suptitle(labels[m])
    for row, axis, gt in zip(range(3), ["vx", "vy", "vz"], [gt_vx, gt_vy, gt_vz]):
        ax_gt, ax_est = axs[row, 0], axs[row, 1]
        ax_gt.plot(frames, gt, color="black", label="Ground Truth")
        ax_est.plot(frames, vel[f"{axis}_{m}"].values[:n], color="tab:blue", label=labels[m])
        ax_gt.set_ylabel(f"V{axis[1]} (m/s)")
        ax_gt.legend()
        ax_est.legend()
    axs[0, 0].set_title("Ground Truth")
    axs[0, 1].set_title(labels[m])
