...
# Paths
LEFT_VIDEO  = r"D:\Stingray Robotics\Homegrown Data\camera1.mp4"
RIGHT_VIDEO = r"D:\Stingray Robotics\Homegrown Data\camera2.mp4"
CALIB       = r"D:\Stingray Robotics\Homegrown Data\calibC.txt"
VELOCITY_CSV_PATH = r"D:\Stingray Robotics\velocity_outputE.csv"
POSITION_CSV_PATH = r"D:\Stingray Robotics\position_outputE.csv"

# IMU orientation (fused quaternion, one component per file, all sharing the same
# "elapsed time" column) -- adjust these paths to wherever you exported them
IMU_ORIENTATION_W_CSV = r"D:\Stingray Robotics\Homegrown Data\Final Dataset\IMU_Orientation\imu_or_w.csv"
IMU_ORIENTATION_X_CSV = r"D:\Stingray Robotics\Homegrown Data\Final Dataset\IMU_Orientation\imu_or_x.csv"
IMU_ORIENTATION_Y_CSV = r"D:\Stingray Robotics\Homegrown Data\Final Dataset\IMU_Orientation\imu_or_y.csv"
IMU_ORIENTATION_Z_CSV = r"D:\Stingray Robotics\Homegrown Data\Final Dataset\IMU_Orientation\imu_or_z.csv"
  
MIN_FEATURES = 400

# Set to an integer to stop after that many frames (useful for quick test runs);
# set to None to process the full video as before
MAX_FRAMES = None
...
# Load IMU Orientation (fused quaternion, replaces vision-derived angular
# velocity integration -- avoids the heading drift that comes from
# accumulating noisy per-frame rotation estimates)


imu_w_df = pd.read_csv(IMU_ORIENTATION_W_CSV)
imu_x_df = pd.read_csv(IMU_ORIENTATION_X_CSV)
imu_y_df = pd.read_csv(IMU_ORIENTATION_Y_CSV)
imu_z_df = pd.read_csv(IMU_ORIENTATION_Z_CSV)

# All 4 files share the same "elapsed time" column (seconds since bag start)
imu_t    = imu_w_df["elapsed time"].values
imu_quat = np.stack([
    imu_w_df["value"].values,
    imu_x_df["value"].values,
    imu_y_df["value"].values,
    imu_z_df["value"].values,
], axis=1)  # columns: [w, x, y, z]

# Fixed rotation from IMU frame to camera frame (physical mounting offset).
# Identity assumes the IMU and camera axes are already aligned -- replace this
# with your measured/calibrated cam-IMU extrinsic rotation for correct results.
R_cam_imu = np.eye(3)

def get_R_world_for_time(t_seconds):
    """Look up the closest IMU orientation sample for a given video time
    (assumes video and IMU recording started at the same instant, i.e. rosbag
    time-sync) and return it as a rotation matrix, camera-frame -> world-frame."""
    idx = np.searchsorted(imu_t, t_seconds)
    idx = np.clip(idx, 0, len(imu_t) - 1)
    q = imu_quat[idx]  # [w, x, y, z]
    R_imu = Rot.from_quat([q[1], q[2], q[3], q[0]]).as_matrix()  # scipy wants [x,y,z,w]
    return R_cam_imu @ R_imu
...
ret_l, left_frame_old_color  = cap_left.read()
ret_r, right_frame_old_color = cap_right.read()
left_frame_old  = cv2.cvtColor(left_frame_old_color,  cv2.COLOR_BGR2GRAY)
right_frame_old = cv2.cvtColor(right_frame_old_color, cv2.COLOR_BGR2GRAY)
...
# No ground truth is available, so orientation is integrated from the estimated
# angular velocity (wx, wy, wz) that compute_velocity already solves for, and
# position starts at the origin.
...
# Integrated rotation, one per feature front-end (shared by its MA/LP variants)
# NOTE: no longer accumulated frame-by-frame -- each frame this gets overwritten
# by the IMU orientation lookup (see get_R_world_for_time). Kept as the initial
# value in case the very first IMU lookup were ever to fail.
R_sp   = np.eye(3)
R_sift = np.eye(3)
...
while True:
    ...
    ret_l, left_frame_new_color  = cap_left.read()
    ret_r, right_frame_new_color = cap_right.read()
    ...
    left_frame_new = cv2.cvtColor(left_frame_new_color,  cv2.COLOR_BGR2GRAY)
    left_img       = left_frame_new
    right_img      = cv2.cvtColor(right_frame_new_color, cv2.COLOR_BGR2GRAY)
    ...
    # findFundamentalMat's RANSAC can crash (OpenCV rowRange assertion) on near-degenerate
    # point sets close to the 8-point minimum, so require some margin above it, and still
    # try/except as a safety net in case a later frame trips it anyway.
    if len(sp_old) >= 15:
        try:
    ...
    # Same margin + try/except guard as the SuperPoint branch above.
    if len(sift_old) >= 15:
        try:
    ...
    # Orientation Integration (replaces Ground Truth rotation)
    # NOTE: this used to integrate the noisy vision-derived angular velocity
    # (wx, wy, wz) frame by frame, which accumulates heading drift over a long
    # run. Commented out below, replaced with a direct per-frame lookup into
    # the IMU's fused orientation, which is an absolute measurement each frame
    # rather than something built up by accumulation, so it doesn't drift.

    # delta_R_sp,   _ = cv2.Rodrigues(np.array([wx_sp,   wy_sp,   wz_sp])   * dt)
    # delta_R_sift, _ = cv2.Rodrigues(np.array([wx_sift, wy_sift, wz_sift]) * dt)
    # R_sp   = R_sp   @ delta_R_sp
    # R_sift = R_sift @ delta_R_sift

    frame_time_sec = frame_idx / fps
    R_sp = R_sift = get_R_world_for_time(frame_time_sec)


    # Dead Reckoning: camera-frame velocity -> world-frame velocity (via integrated R) -> position (x = x + v*dt)


    pos_sp_ma   += (R_sp   @ np.array([vx_sp_ma,   vy_sp_ma,   vz_sp_ma]))   * dt
    pos_sp_lp   += (R_sp   @ np.array([vx_sp_lp,   vy_sp_lp,   vz_sp_lp]))   * dt
    pos_sift_ma += (R_sift @ np.array([vx_sift_ma, vy_sift_ma, vz_sift_ma])) * dt
    pos_sift_lp += (R_sift @ np.array([vx_sift_lp, vy_sift_lp, vz_sift_lp])) * dt
